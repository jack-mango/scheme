...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        evaluated_op = scheme_eval(first, env)
        validate_procedure(evaluated_op)   
        return scheme_apply(evaluated_op, rest.map(lambda argument: scheme_eval(argument, env)), env)
        # END PROBLEM 4
...
def do_or_form(expressions, env):
    ...
    if not expressions:
        return False
    while expressions:
        evaluated_first = scheme_eval(expressions.first, env)
        if is_true_primitive(evaluated_first):
            return evaluated_first
        else:
            expressions = expressions.rest
    return False
    # END PROBLEM 12
